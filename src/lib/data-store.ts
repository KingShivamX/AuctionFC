...
// USER OPERATIONS
export async function registerUser(username: string, teamName: string, pin: string): Promise<User> {
  ...
  if (db) {
    ...
    try {
      await users.insertOne({ ...newUser, usernameKey: cleanUsernameKey } as any);
    } catch (error: any) {
      if (error?.code === 11000) {
        throw new Error("That username is already taken. Please choose another.");
      }
...
export async function getLeagueByCodeOrId(codeOrId: string): Promise<League | null> {
...
export async function joinLeague(leagueCode: string, user: User): Promise<League> {
  const league = await getLeagueByCodeOrId(leagueCode);
  if (!league) {
    throw new Error("League code not found");
  }

  const alreadyMember = league.members.some((m) => m.userId === user.id || m.username === user.username);
...
export async function getLeaguePlayers(leagueId: string): Promise<Player[]> {
  ...
  return Object.values(memoryStore.players)
    ...
    .sort((a: any, b: any) => a.orderIndex - b.orderIndex || a.srNo - b.srNo) as Player[];
}

export async function getBidsForPlayer(leagueId: string, playerId: string): Promise<Bid[]> {
  const db = await getDb();
  if (db) {
    const docs = await db
      .collection("bids")
...
export async function placeBid(
  ...
  amount: number,
  user: User
): Promise<{ success: boolean; bid?: Bid; currentHighest?: number; message?: string }> {
  const league = await getLeagueByCodeOrId(leagueId);
  if (!league) return { success: false, message: "League not found" };

  if (league.isPaused) {
    return { success: false, message: "Auction is currently paused by admin" };
  }

  if (league.currentAuctionPlayerId !== playerId) {
    return { success: false, message: "This player is not currently active on the auction block" };
  }

  const players = await getLeaguePlayers(leagueId);
  const player = players.find((p) => p.id === playerId);
  if (!player || player.soldStatus !== "live") {
    return { success: false, message: "Player bidding has ended or is invalid" };
  }

  // Calculate team remaining purse
  const teamSpent = players
    .filter((p) => p.soldStatus === "sold" && p.soldToUsername === user.username)
  ...
  const currentHighest = bids.length > 0 ? bids[0].amount : player.basePrice - 0.01;
...
// ADMIN ACTIONS
export async function startNextAuctionPlayer(leagueId: string, targetPlayerId?: string): Promise<Player | null> {
  const players = await getLeaguePlayers(leagueId);
...
export async function pauseResumeAuction(leagueId: string, pause?: boolean): Promise<League | null> {
  const league = await getLeagueByCodeOrId(leagueId);
  ...
  if (shouldPause) {
    ...
    pausedTimeLeft = Math.max(0, Math.ceil(((league.timerEnd || Date.now()) - Date.now()) / 1000));
  } else {
  ...
  if (db) {
    await db.collection("leagues").updateOne(
      { id: leagueId },
      { $set: { isPaused: shouldPause, timerEnd: updatedTimerEnd, pausedTimeLeft } }
    );
  } else {
    memoryStore.leagues[leagueId] = league;
  }
...
export async function finalizeSoldPlayer(leagueId: string, playerId: string): Promise<{ player: Player; soldTo?: string; amount?: number }> {
  const bids = await getBidsForPlayer(leagueId, playerId);
  const players = await getLeaguePlayers(leagueId);
  const player = players.find((p) => p.id === playerId);
...
export async function undoDirectAssignPlayer(leagueId: string, playerId: string): Promise<Player> {
  const players = await getLeaguePlayers(leagueId);
  const player = players.find((p) => p.id === playerId);
  if (!player) throw new Error("Player not found");
...
function setPlayerSoldStatus(leagueId: string, playerId: string, status: 'unsold' | 'live' | 'sold') {
  return getDb().then((db) => {
    if (db) {
      return db.collection("players").updateOne({ id: playerId }, { $set: { soldStatus: status } });
    } else {
      if (memoryStore.players[playerId]) memoryStore.players[playerId].soldStatus = status;
    }
  });
}
